...
class LabelConnector:
    ...
    def applyStyle(self, expressionFile):
        try:
            with open(expressionFile, 'r') as f:
                ...
                if self.isLayerContainsLabelConnector():
                    ret = QMessageBox.question(
                        None,
                        self.tr("LabelConnector already exists"),
                        self.tr(
                            "Do you want to replace the existing LabelConnectore?")
                    )
                    if ret == QMessageBox.Yes:
                        self.replaceCurrentLabelConnector(expr)
                        self.messageBar.pushInfo(self.tr("Success"), self.tr(
                            "Label connector successfully replaced"))
    ...
    def isLayerContainsLabelConnector(self):
        renderer = self.layer.renderer()
        symbolType = renderer.type()
        ...
        if symbolType in ("nullSymbol", "invertedPolygonRenderer", "25dRenderer", "grassEdit", "heatmapRenderer", "pointCluster", "pointDisplacement"):
            ...
            sym = renderer.symbol()
            return self.isSymbolContainsLabelConnector(sym)
            ...
            for sym in renderer.symbols(QgsRenderContext.fromMapSettings(self.iface.mapCanvas().mapSettings())):
                if self.isSymbolContainsLabelConnector(sym):
                    return True
            return False
            ...
            print(isLabeledList)
            if any(isLabeledList) and len(isLabeledList) > 0:
                return True
            else:
                return False
    ...
    def replaceSymbolLabelConnector(self, symbol, expression):
        for sl in symbol.symbolLayers():
            if type(sl) is QgsGeometryGeneratorSymbolLayer:
                sl.setGeometryExpression(expression)

    def replaceCurrentLabelConnector(self, expression):
        renderer = self.layer.renderer()
        symbolType = renderer.type()
        ...
        if symbolType in ("nullSymbol", "invertedPolygonRenderer", "25dRenderer", "grassEdit", "heatmapRenderer", "pointCluster", "pointDisplacement"):
            ...
            sym = renderer.symbol()
            self.replaceSymbolLabelConnector(sym, expression)
            return True
            ...
            for c in renderer.rootRule().children():
                sym = c.symbol()
                if sym is not None:
                    self.replaceSymbolLabelConnector(sym, expression)
            return True
